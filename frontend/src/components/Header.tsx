...
const Header: React.FC<HeaderProps> = ({
  ...
  >
    <div className="flex items-center gap-4">
      ...
      {error && (
        <span className="ml-2 text-xs text-red-300" role="alert" aria-live="assertive">{error}</span>
      )}
      {/* LLM Provider Status */}
      <div className="flex items-center gap-2 ml-6">
        ...
        <span className="text-xs font-semibold" aria-label="LLM Provider">{llmProvider}</span>
        {llmState && <span className="text-xs opacity-70 ml-1">{llmState}</span>}
      </div>
    </div>
    <div className="flex items-center gap-4">
